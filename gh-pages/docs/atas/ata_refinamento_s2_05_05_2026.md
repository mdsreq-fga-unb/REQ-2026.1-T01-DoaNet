# Refinamento do User Story Map — Sprint 2

**Data:** 05/05/2026  
**Local:** Discord  
**Participantes:** Davi Ursulino, João Leles, Letícia Vitória, Pedro Augusto e Pedro Druck

## Pauta

- Revisão e replanejamento do User Story Map após o pivoteamento realizado em 04/05
- Repriorização das histórias de usuário com a nova stack (FastAPI + MongoDB + Flutter + Streamlit)
- Detalhamento das histórias selecionadas para a segunda semana da sprint (US17, US18, US19)
- Distribuição de tarefas entre os membros para a entrega do CRUD de posts

## Encaminhamentos

| Atividade | Responsável | Prazo |
| :--- | :--- | :--- |
| Configuração da arquitetura base do back-end em FastAPI + MongoDB | Pedro Druck | 12/05/2026 |
| Implementação dos endpoints de CRUD do feed (US17, US18, US19) | Pedro Augusto | 12/05/2026 |
| Setup inicial do projeto Flutter e integração com o back-end | Pedro Druck e Letícia Vitória | 12/05/2026 |
| Levantamento e planejamento da estrutura do módulo administrativo (Streamlit) — a ser iniciado na Sprint 3 | João Leles | 12/05/2026 |
| Implementação das telas do feed no front-end (criação, edição e deleção de posts) | Letícia Vitória | 12/05/2026 |
| Apoio na integração front-back e testes manuais do CRUD do feed | Davi Ursulino | 12/05/2026 |
| Preparação da demonstração do incremento para a validação com o cliente | Letícia Vitória | 12/05/2026 |
