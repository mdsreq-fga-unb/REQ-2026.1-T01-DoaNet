# Sprint 0 — Definição do Projeto · Objetivo Geral

**Período:** 31/03/2026 – 14/04/2026  
**Status:** ✅ Concluída

## Descrição Geral

Sprint inicial de definição do projeto DoaNet. O foco foi o entendimento do domínio (terceiro setor / ONGs), o levantamento das necessidades reais junto ao cliente e a estruturação técnica de partida, estabelecendo a base para as sprints de desenvolvimento.

## Objetivos da Sprint

- Definição do projeto e do problema a resolver.
- Levantamento das necessidades iniciais junto ao cliente.
- Definição da arquitetura e da stack tecnológica inicial.

## Sumário da Sprint

| Seção | Conteúdo |
| :--- | :--- |
| [User Stories](user-stories.md) | Histórias trabalhadas na sprint (nenhuma US funcional nesta sprint) |
| [Engenharia de Requisitos](engenharia-de-requisitos.md) | Atividades de ER realizadas e suas evidências |
| [Reuniões Realizadas](reunioes-realizadas.md) | Cerimônias do Scrum, atas e descrições |
| [Engenharia de Software](engenharia-de-software.md) | DoR, DoD e descrição da entrega técnica |

> Referências gerais: [Cenário Atual](../../../visao_produto/1-cenario.md) · [Solução Proposta](../../../visao_produto/2-solucao.md) · [Planejamento (visão geral)](../../../visao_produto/6-planejamento.md)
