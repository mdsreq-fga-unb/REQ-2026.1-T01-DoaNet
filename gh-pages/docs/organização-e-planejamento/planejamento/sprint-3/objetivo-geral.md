# Sprint 3 — Feed Completo, Eventos e Módulo Admin · Objetivo Geral

**Período:** 12/05/2026 – 26/05/2026  
**Status:** ✅ Concluída

## Descrição Geral

Sprint de conclusão do feed (posts de evento com inscrição e suporte a imagens em todos os tipos de post) e de início do módulo de administração com autenticação no Streamlit.

## Objetivo da Sprint

Implementar posts de eventos com inscrição, suporte a imagens em todos os tipos de post e iniciar o módulo de administração.

## Sumário da Sprint

| Seção | Conteúdo |
| :--- | :--- |
| [User Stories](user-stories.md) | US09 e US11 — critérios de aceite, protótipo e rota de acesso |
| [Engenharia de Requisitos](engenharia-de-requisitos.md) | Atividades de ER realizadas e suas evidências |
| [Reuniões Realizadas](reunioes-realizadas.md) | Cerimônias do Scrum, atas e descrições |
| [Engenharia de Software](engenharia-de-software.md) | DoR, DoD e descrição da entrega técnica |
